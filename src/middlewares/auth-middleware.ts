...
function authMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  if (
    refreshToken1Scheme != "Bearer" ||
    refreshToken2Scheme != "Bearer" ||
    accessToken1Scheme != "Bearer" ||
    accessToken2Scheme != "Bearer"
  ) {
    res
      .status(401)
      .json({ message: "fail", error: "unidentified token schema" });
    ...
    if (!userAccessVerified) {
      if (!userRefreshVerified) {
        ...
        if (refreshToken == "undefined" && accessToken == "undefined") {
          ...
          res
            .status(401)
            .json({ message: "fail", error: "all tokens are expired" });
        ...
        Users.findOne({ refreshToken })
          .then((user: IUser | null) => {
            if (user) {
              accessToken = jwt.sign({ userId: user._id }, env.jwt_secret, {
                expiresIn: "1h",
      ...
      if (!userRefreshVerified) {
        ...
        Users.findOne({ _id: userId })
          .then((user: IUser | null) => {
            if (user) {
              refreshToken = jwt.sign({}, env.jwt_secret, {
                expiresIn: "14d",
                issuer: "node-avengers",
              });
          ...
          .then((user: IUser | null) => {
            if (user) {
              if (user.refreshToken != refreshToken) {
                res
                  .status(401)
                  .json({ message: "fail", error: "wrong tokens" });
