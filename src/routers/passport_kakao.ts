    ...
    new Strategy(
      ...
      async function (accessToken, refreshToken, profile, done) {
        ...
        if (
          ...
          profile._json.kakao_account.is_email_valid
        ) {
        ...
        let user = await Users.findOne({
          passport: [{ provider: "kakao" }, { id: userId }],
        });
        ...
        if (!user) {
          user = await Users.create({
            nickname,
            email,
            passport: [{ provider: provider }, { id: userId }],
          });
        ...
        const _id = user._id;
        const refresh = jwt.sign({}, env.jwt_secret, {
          expiresIn: "14d",
          issuer: "node-avengers",
        });
        const access = jwt.sign({ userId: user._id }, env.jwt_secret, {
          expiresIn: "1h",
          issuer: "node-avengers",
        });
