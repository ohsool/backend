  ...
  try {
    ...
    if (
      ...
      beers = await Beers.find({})
        .sort([[sort, -1]])
        .select("hashtag like_array name_korean name_english image");
      ...
      // gives basic order
      beers = await Beers.find({}).select(
        "hashtag like_array name_korean name_english image"
      );
    } else {
...
const postBeer = async (req: Request, res: Response) => {
  try {
    ...
    const new_beer: IBeer = {
      ...
      isDistant,
      categoryId,
      features: beerCategory.features,
      hashtag,
      createDate: date,
    };
...
const unlikeBeer = async (req: Request, res: Response) => {
  ...
  try {
    const exists: Array<IBeer> = await Beers.find({
      _id: beerId,
...
const likedBeer = async (req: Request, res: Response) => {
  try {
    ...
    const likedList: Array<IBeer> = await Beers.find({
      like_array: mongoose.Types.ObjectId(userId),
    });
...
const reportLocation = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const found = locations.some(
      (e) => Array.isArray(e) && e.every((o, i) => Object.is(new_report[i], o))
    );

    if (found) {
      res
        .status(409)
        .send({ message: "fail", error: "location already reported" });
      ...
      if (new_report[0] == location_report[i][0]) {
        if (String(new_report[3]) == String(location_report[i][3])) {
          res
            ...
            .send({
              message: "fail",
              error: "user already reported this location",
            });
    ...
    if (cnt >= 2) {
      // reported 3 times. goes to location.
      for (let i = 0; i < cnt; i++) {
        await Beers.findOneAndUpdate(
          { _id: beerId },
          { $pull: { location_report: new_report } }
        ).lean();
      }

      await Beers.findOneAndUpdate(
        { _id: beerId },
        { $push: { location: [new_report] } }
      ).lean();
    } else {
      // reported less than 3 times. goes to location_report.
      await Beers.findOneAndUpdate(
        { _id: beerId },
        { $push: { location_report: [new_report] } }
      ).lean();
    }
...
const getAllFeatures = async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (let i = 0; i < beers.length; i++) {
      const myBeers = await MyBeers.find({ beerId: beers[i]._id }).select(
        "myFeatures -_id"
      );
      ...
      if (count == beers[i].calculatedCount! || count == 0) {
        // same as before
        continue;
      }
      ...
      const beer = await Beers.findOneAndUpdate(
        { _id: beers[i]._id },
        { $set: { features: newFeatures, calculatedCount: count } }
      ).select("name_korean features");
...
const getFeatures = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!myBeers) {
      res
        .status(204)
        .json({ message: "fail", error: "no mybeers with this beer" });
    ...
    const res_beer = await Beers.findOneAndUpdate(
      { _id: beerId },
      { $set: { features: newFeatures, calculatedCount: count } }
    ).select("name_korean features");
