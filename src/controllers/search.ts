...
const search = async (req: Request, res: Response) => {
  ...
  try {
    if (word) {
      ...
      for (let i = 0; i < beers.length; i++) {
        if (
          getRegExp(word.replace(/\s+/g, "")).test(
            beers[i].name_korean.replace(/\s+/g, "")
          )
        ) {
          // korean beer name
          searched_beers.push(beers[i]);
        } else if (
          beers[i].name_english.replace(/\s+/g, "").toLowerCase().includes(word)
        ) {
          // english beer name
          searched_beers.push(beers[i]);
        }
      ...
      for (let i = 0; i < beerCategories.length; i++) {
        // category names only have english name
        if (
          beerCategories[i].name
            .replace(/\s+/g, "")
            .toLowerCase()
            .includes(word)
        ) {
    ...
    res.json({
      message: "success",
      beers: searched_beers,
      beerCategories: searched_categories,
      hashtags: send_hashtags,
    });
  } catch (error) {
