...
it("login success", async () => {
  ...
  const beer: IBeer | null = await Beers.findOne({
    name_korean: "버드와이저",
  })!;
  beerId = beer!._id!;
...
it("post mybeer - success", async () => {
  ...
  let beerCategory: IBeerCategory | null = await BeerCategories.findById(
    beerCategoryId
  );
  ...
  const avgRateCategory =
    (countCategoryBefore * avgRateCategoryBefore + rate) /
    (countCategoryBefore + 1);
...
it("post mybeer - fail (no rate)", async () => {
  ...
  expect(response.body.error).toBe(
    "Either beer or myFeatures or rate doesn't exist."
  );
});
...
it("post mybeer - fail (review too long)", async () => {
  const response = await request(app)
    ...
    .send({
      ...
      rate,
      review:
        "test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long test mybeer review too long",
    });

  expect(response.body.message).toBe("fail");
  expect(response.body.error).toBe(
    "the length of the review must be under 300"
  );
});
...
it("modify one mybeer - success", async () => {
  ...
  let beerCategory: IBeerCategory | null = await BeerCategories.findOne({
    _id: beer!.categoryId,
  });
  const avgRateCategoryBefore = beerCategory!.avgRate!["Unknown"][0];
  const countCategoryBefore = beerCategory!.avgRate!["Unknown"][1];
  ...
  const avgRate =
    (+countBefore * +avgRateBefore - rate + modified_rate) / +countBefore;
  const avgRateCategory =
    (countCategoryBefore * avgRateCategoryBefore - rate + modified_rate) /
    countCategoryBefore;
...
it("delete one mybeer - success", async () => {
  ...
  if (countBefore > 1) {
    avgRate =
      (+countBefore * +avgRateBefore - modified_rate) / (+countBefore - 1);
    avgRateCategory =
      (countCategoryBefore * avgRateCategoryBefore - modified_rate) /
      (countCategoryBefore - 1);
  }
